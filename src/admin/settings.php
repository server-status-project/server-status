...
<?php } ?>
<section>
    <h3 class="float-start"><?php echo _('Services'); ?></h3>
    <?php if ($user->get_rank() <= 1) { ?>
    <form action="?do=settings&new=service" method="post">
        <div class="float-end new-service">
            <a href="<?php echo WEB_URL; ?>/admin/?do=new-service" class="btn btn-success float-end"><?php echo _('Add new service'); ?></a>
        </div>
    </form>
    <?php } ?>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                while ($result = $query->fetch_assoc()) {
                  ...
                  if ($user->get_rank() <= 1) {
                    echo '<td><a href="'.WEB_URL.'/admin/?do=settings&type=service&delete='.$result['id'].'" class="float-end delete-service"><i class="fa fa-trash"></i></a></td>';
                  }
...
<section>
    <h3 class="float-start"><?php echo _('Services Groups'); ?></h3>
    <?php if ($user->get_rank() <= 1) { ?>
    <form action="?do=settings&new=service-group" method="post">
        <div class="float-end new-service">
            <a href="<?php echo WEB_URL; ?>/admin/?do=new-service-group" class="btn btn-success float-end"><?php echo _('Add new service group'); ?></a>
        </div>
    </form>
    <?php } ?>
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                ...
                while ($result = $query->fetch_assoc()) {
                  ...
                  if ($user->get_rank() <= 1) {
                    echo '<td><a href="'.WEB_URL.'/admin/?do=settings&type=groups&delete='.$result['id'].'" class="float-end delete-service"><i class="fa fa-trash"></i></a></td>';
                  }
...
<section>
    <h3 class="float-start"><?php echo _('Users'); ?></h3>
    <?php
    if ($user->get_rank() == 0) {
      ?>
         <a href="<?php echo WEB_URL; ?>/admin/?do=new-user" class="btn btn-success float-end"><?php echo _('Add new user'); ?></a>
      <?php
    }
    ?>
    <div class="table-responsive">
