...
class Mailer
{
  ...
  public function send_mail($to, $subject, $message, $html = true)
  {
    ...
    if ($this->is_utf8($to)) {
      ...
      $domainpart = $punycode->encode(array_pop($elements));
        // Convert domain part to ascii
      $to = $elements[0].'@'.$domainpart;
        // Reassemble tge full email address
    }

      // Send using PHP mailer if it is enabled
    if (PHP_MAILER) {
      include_once PHP_MAILER_PATH.'/Exception.php';
        // Exception class.
      include_once PHP_MAILER_PATH.'/PHPMailer.php';
        // The main PHPMailer class.
      if (PHP_MAILER_SMTP) {
        include_once PHP_MAILER_PATH.'/SMTP.php';
  ...
  public function verify_domain($email)
  {
    ...
    $punycode = new Punycode();
    $domain = $punycode->encode(array_pop($domain).'.');
      // Add dot at end of domain to avoid local domain lookups
    syslog(1, $domain);
  ...
  public function convert_html_to_plain_txt($content, $remove_links = false)
  {
    ...
    $plain_text = preg_replace(
      ...
      // Remove everything from between the tags that doesn't get removed with strip_tags function
