...
class Notification
{
  ...
  public $status = '';
     ...
     *
     * @param  integer $status_id The incident to query
     * @return boolean
     */
  ...
  }//end populate_impacted_services()
  ...
  public function notify_subscribers()
  {
    ...
    $arr_data = [];
    if (SUBSCRIBE_EMAIL) {
      $arr_data = $this->prepare_email();
        // Make up the base message and subject for email
    ...
    if (SUBSCRIBE_TELEGRAM) {
      ...
      $queue->add_notification($arr_telegram);
        // Add array of Telegram users to the notification queue list
    }

    if (SUBSCRIBE_EMAIL) {
      $queue->task_id = $task_id_email;
      $queue->add_notification($arr_email);
        // Add array of Email users to the notification queue list
    }
  ...
  public static function submit_queue_telegram($userID, $firstname, $msg)
  {
     ...
     * @return void
     */
  public static function submit_queue_email($subscriber, $subject, $msg)
  {
      // TODO Error handling
    $mailer = new Mailer();
    ...
    return true;
  }//end submit_queue_email()


  public function prepare_email()
  {
    $Parsedown = new Parsedown();
    $str_mail  = file_get_contents('../templates/email_status_update.html');
