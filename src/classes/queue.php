...
class Queue
{
  ...
  public static function delete_task($task_id)
  {
     ...
     *
     * @param  integer $new_status The new current status of the task. Must be selected from the $all_status array.
     * @return void
     */
  ...
  public function add_notification($arr_data)
  {
    ...
    $this->set_task_status($this->all_status['ready']);
        // Make task available for release
  ...
  public static function update_notification_retries($task_id, $subscriber_id)
  {
    ...
    $stmt->execute();
  }//end update_notification_retries()
    ...
    while ($result = $stmt->fetch_assoc()) {
      ...
      if ($typeID == 1) {
        ...
        if (! Notification::submit_queue_telegram($result['userID'], $result['firstname'], $msg)) {
          self::update_notification_retries($result['task_id'], $result['subscriber_id']);
            // Sent
        } else {
          self::delete_notification($result['task_id'], $result['subscriber_id']);
              // Failed
        }
      }
