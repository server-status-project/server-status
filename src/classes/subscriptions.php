...
class Subscriptions
{
  ...
  function render_subscribed_services($typeID, $subscriberID, $userID, $token)
  {
    global $mysqli;
    $stmt = $mysqli->prepare(
      'SELECT services.id, services.name, subscribers.subscriberID, subscribers.userID, subscribers.token
                                  FROM services
        ...
        <div class="row">
          <div class="col-xs-12 offset-lg-2 col-lg-8">
            <div class="text-center">
              ...
              <p>
    <?php
    echo _('Manage notification subscription for');
    echo '&nbsp'.$userID;
    ?>
                </p>              
    ...
    $subs = [];
        // Will be used to hold IDs of services already selected
    if ($query->num_rows) {
      while ($result = $query->fetch_assoc()) {
        echo '<a href="'.WEB_URL.'/subscriptions.php?remove='.$result['id'].'" class="list-group-item"><span class="bi bi-x  text-danger"></span>&nbsp;'.$result['name'].'</a>';
        $subs[] = $result['id'];
      }
    } else {
    ...
    if ($query->num_rows) {
      while ($result = $query->fetch_assoc()) {
        echo '<a href="'.WEB_URL.'/subscriptions.php?add='.$result['id'].'" class="list-group-item list-group-item-action"><span class="bi bi-plus  text-success"></span>&nbsp;'.$result['name'].'</a>';
      }
    } else {
    ...
    echo '</div>';
  }//end render_subscribed_services()
}//end class
