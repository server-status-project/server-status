...
define('WEB_URL', '.');
// Website name
define('NAME', _('Status page'));
// Website name
define('MINIMUM_PHP_VERSION', '5.4.0');
define('POLICY_URL', 'policy.php');
// Default policy URL
...
if (isset($_POST['server']) && empty($message)) {
  ...
  if (empty($message)) {
    ...
    $db->setSetting($mysqli, 'url', rtrim(trim($_POST['url']), "/"));
    ...
    $policy_url_conf = ( ! empty($_POST['policy_url']) ) ? htmlspecialchars($_POST['policy_url'], ENT_QUOTES) : rtrim(trim($_POST['url']), "/").'/policy.php';
    ...
    <section class="prereq-section clearfix">
        <div class="container-fluid">
            <div class="row ">
                ...
                <div class="col-md-6"><a href="#" class="btn btn-<?php echo $preq_phpver[1]; ?>"><span class="bi bi-<?php echo $preq_phpver[0]; ?>"></span></a></div>
            </div>
            <div class="row ">
                <div class="col-md-6 text-right"><label for="mysqlnd Library installed"><?php echo _('PHP mysqlnd library installed'); ?></div>
                <div class="col-md-6"><a href="#" class="btn btn-<?php echo $preq_mysqlnd[1]; ?>"><span class="bi bi-<?php echo $preq_mysqlnd[0]; ?>"></span></a></div>
            </div>
            <div class="row">
                <div class="col-md-6 text-right"><label for="write_access"><?php echo _('Write access to web directory'); ?></div>
                <div class="col-md-6"><a href="#" class="btn btn-<?php echo $preq_writedir[1]; ?>"><span class="bi bi-<?php echo $preq_writedir[0]; ?>"></span></a></div>
            </div>
        </div>
...
<form method="post" action="." class="clearfix install">
    ...
    <section class="install-section clearfix">
        ...
        <div class=" clearfix">
            <div class="col-sm-12"><label for="who_we_are"><?php echo _('Who we are'); ?>: </label><textarea class="form-control" id="who_we_are" rows="3" name="who_we_are" placeholder="<?php echo _('Some info about yourself'); ?>" value="<?php echo (isset($_POST['who_we_are'])) ? htmlspecialchars($_POST['who_we_are'], ENT_QUOTES) : ''; ?>"></textarea></div>
        </div>
        <div class=" clearfix">
