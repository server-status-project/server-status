(function () {
    $("#time_input").flatpickr(
        {enableTime:true, minDate: "today",time_24hr:true, onChange: function (date, dateStr, instance) {
    ...
    var classes = ["card card-danger", "card card-warning", "card card-primary", "card card-success"];
    var icons = ["fa fa-times", "fa fa-exclamation", "fa fa-info", "fa fa-check"];

    $("body").on(
        "change","#new-incident select", function () {
            var val = parseInt($(this).val()); //this should make exploitation harder

            $("#new-incident .card.new .card-header i").get(0).className = icons[val];
            $("#new-incident .card.new").get(0).className = classes[val] + " new";
            $("#time_input").val("");
            $("#end_time").val("");
        }
    );
    ...
    $("body").on(
        "submit","#new-incident",function () {
            ...
            if ($(".list-group.components :checkbox:checked").length === 0) {
                $.growl.error({ message: "Please check at least one service!" });
                $(".list-group.components").addClass("error");
                return false;
            }
        }
    );
